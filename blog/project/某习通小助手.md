...
这是我当时编写软件前在吾爱破解论坛上搜索到的相关源码，如下：![image-20200925173801271](assert/e561b490bfee799369e0cdfcc5ddb7fd_MD5.png)

看看软件源码的界面![image-20200925173832002](assert/8918291f49ca4ef223b487a0912a6994_MD5.png)

在比对一下我的修改了数十次的。

![image-20201220072514909](assert/6c93e156c07d7b5b2fa6408900c364f4_MD5.png)
...
![image-20201226062612827](assert/824e1b79c0f14e3f356a572766f03ea4_MD5.png)
...
![image-20201226063607140](assert/a311804530938a76c849de2ca1fde3d7_MD5.png)

具体看图片

![image-20201226064003045](assert/e0005114a13d8a9beff33099d1ccd5ab_MD5.png)
...
![](assert/c3ccce3bf38e890960b1fd04b1034347_MD5.png)
...
![image-20210102035545981](assert/1c62485deeb5f7d85f50dea2476e9122_MD5.png)
...
![image-20210102035927028](assert/ffb696613b102e9cdeca939403ba6252_MD5.png)
...
![image-20210102040540378](assert/da812ae4709a83e2297e900569687325_MD5.png)
...
![image-20210102041501299](assert/74eebee8a1695263d1013692b2f251e9_MD5.png)

添加了注释，就懒得在打字一个个说明了，执行逻辑并不难，也就是判断，然后执行，接着要到刷视频和题这个方法，因为最主要还是这个代码在干什么。（后面也会将写上代码注释，方便大家理解）

#### 开始刷视频和题

![image-20210102044334946](assert/cc0a07a4790f714b42740117ff7059c5_MD5.png)

开始循环访问选择夹，接下来代码有点多，执行流程也就是循环，判断，我简化了很多，认真看

![image-20210102050722756](assert/b8b2609be5e162f12ef78185bf4c150b_MD5.png)
...
![image-20210102074018606](assert/e218c41dd06faac80992e57bfbd2fb9f_MD5.png)

取到我们想要的数据，并存为变量即可，接着才是关键所在，获取到了课件信息，同时判断课件类型为视频，并且视频的通过状态为 false，那么接下来就是要提交视频了。相关代码如下

#### 提交视频

![image-20210102052457877](assert/18e4a9126cab9da8ecc825d20a1e2b58_MD5.png)
...
![image-20210102073628331](assert/e9d378185dca7fe79318a0a496a20cda_MD5.png)
...
![ABCWF](assert/aae64cb400b6be9d65d598036dcd8e4a_MD5.jpg)

那么我先把干扰线去掉，并且二值化处理一下，变成下面这样

![image-20200924232042428](assert/534bb7c0c3c84f68c7d71d61d32ea6d8_MD5.png)

然后这时候开始抠图，抠出 A，B，C，W，F 字符，比如抠出 A 字符

![image-20200924232357263](assert/f9de7ac278ca8592c6f5c3a7c3f04777_MD5.png)

接着，做几百张这样的抠图图片，如

![demo4](assert/e1bf76923c37e5e1eaae899e0368c706_MD5.gif)
...
![image-20200924233820001](assert/cb1f14566160993f4a7dbf829ff36700_MD5.png)

甚至还看了一个图象识别的教程手写一个类似这样的识别系统，最终效果如下图。

![demo5](assert/ee9ffe5c28966f0a4a39beac86f97fcc_MD5.gif)
...
![image-20200926161606528](assert/df04944dd004305702435566d4b9b4d8_MD5.png)

你需要注册一个账号使用，如果时间到期了就需要充值卡密使用，但你如果不注册的话，你就无法使用这个软件，能有效的防止破解者，提高破解门槛（该破解都能破解，就看想不想了），这是一个客户端，对应的肯定有服务端

![image-20200926161958435](assert/b9df48b930f6d745638f5aa6b954645b_MD5.png)
