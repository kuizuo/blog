---
id: brief-talk-encrypt-algorithm
slug: /brief-talk-encrypt-algorithm
title: 浅谈加密算法
date: 2020-09-02
tags:
  - 逆向
...
玩笑归玩笑，这类似的例子外面也都常说，但可能初识消息摘要算法的同学可能会疑惑，为什么好端端的数据通过这个算法后就变不回去了。而这正是这个算法的最重要的部分，我这简而概之一下，就比如`kuizuo`这个字符串 通过 MD5 算法后得到的结果是`ff1fa96799ded9ee89d0f764b3e9ff54` ，在算法内部会将 `kuizuo`转成字节数组，然后**删除指定部分区域**，然后再重新将删除后剩余的部分重新通过 hex 编码后得到的结果给你，删除后的字符自然无法还原成原先的字符串，也就不可逆，而至于怎么个实现原理（别问，问就是不知道）

但是这还远远不够，由于密文是固定的，理论上只要我一个一个字符是过去是不是就可以得到原文，那么我能不能先把字符串全部通过 MD5 加密一遍，得到对应的结果，然后将这些原文和密文存入对应的数据库里，在查加密后的密文后去数据库找原文，变相的加密，理论上是可以的，如彩虹表就是专门暴力破解这种算法的(相关链接 [什么是彩虹表](https://www.zhihu.com/question/19790488))。这时候就需要对原文在做一次操作----加盐。加盐可以理解为就是添加了一些字符串，例如上面说到`kuizuo`这个字符串 通过 MD5 算法后得到的结果是`ff1fa96799ded9ee89d0f764b3e9ff54` 这就是不加盐 MD5 返回的结果，万一我在`kuizuo`后面加一个`！`后呢，结果为`ad0a3484ea7c99b5b71eb68d3b278ce6`，你可以看到两者完全天壤之别，这就是这个算法妙的地方，我不加`!`的彩虹表所对应的`kuizuo`密文可能在彩虹表里都有了，但是我这样处理，在`kuizuo`这个字符串前或后，都加一个随机的字符串（这些随机的字符串可要记得，不然你自己到时候数据效验的时候也不知道原文对不对了），然后进行拼接在取 MD5 的值，这样他的彩虹表就废了，就需要猜测出我的加的盐，然后在重新生成密文与原文对应的数据库。总之别单纯的只 MD5，加一些盐来进行“混淆”处理。
...
顺便一提，那么这是发送手机验证码的，我现在也得到了 sign 算法是不是就能做个伪短信轰炸？是的，外面的短信轰炸原理也就是这个，我这边就放一张图片吧 通常很多小平台的发送短信都不用上面那个 sign
...
假设我随机生成的密钥 key 为`0123456789abcdef`，明文是`kuizuo`。首先用 AES/EBC/PKCS5Padding 填充将明文加密得到`gujzY/9NeeM=`，然后通过 RSA/None/NOPadding 公钥 puk 加密得到加密结果``，然后将这两串数据发送给服务器，如：
