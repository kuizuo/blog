...
import type { PluginOptions, PropTagsListPage } from '@docusaurus/plugin-content-docs'
...
async function docsPluginEnhanced(context: LoadContext, options: PluginOptions): Promise<Plugin<LoadedContent>> {
  ...
  return {
    ...
    async contentLoaded({ content, actions }) {
      ...
      const tagsProp: PropTagsListPage['tags'] = Object.values(versionTags).map((tagValue) => ({
